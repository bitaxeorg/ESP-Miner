#include "bm1373.h"

#include "crc.h"
#include "global_state.h"
#include "serial.h"
#include "utils.h"

#include "esp_log.h"
#include "freertos/FreeRTOS.h"
#include "freertos/task.h"
#include "frequency_transition_bmXX.h"
#include "pll.h"

#include <stdint.h>
#include <string.h>
#include <arpa/inet.h>

// BM1373 reports 0x1372 as its chip ID over the wire
#define BM1373_CHIP_ID 0x1372
#define BM1373_CHIP_ID_RESPONSE_LENGTH 11

#define TYPE_JOB 0x20
#define TYPE_CMD 0x40

#define GROUP_SINGLE 0x00
#define GROUP_ALL 0x10

#define CMD_SETADDRESS 0x00
#define CMD_WRITE 0x01
#define CMD_READ 0x02
#define CMD_INACTIVE 0x03

#define BM_CHIP_ID 0x00
#define MISC_CONTROL 0x18
#define FAST_UART_CONFIGURATION 0x28

static const register_type_t REGISTER_MAP[] = {
    [0x4C] = REGISTER_ERROR_COUNT,
    [0x88] = REGISTER_DOMAIN_0_COUNT,
    [0x89] = REGISTER_DOMAIN_1_COUNT,
    [0x8A] = REGISTER_DOMAIN_2_COUNT,
    [0x8B] = REGISTER_DOMAIN_3_COUNT,
    [0x8C] = REGISTER_TOTAL_COUNT
};

typedef struct __attribute__((__packed__))
{
    uint32_t nonce;                   // 2-5
    uint8_t midstate_num;             // 6
    uint8_t id;                       // 7
    uint16_t version;                 // 8-9
} bm1373_asic_result_job_t;

typedef struct __attribute__((__packed__))
{
    uint32_t value;                   // 2-5
    uint8_t asic_address;             // 6
    uint8_t register_address;         // 7
    uint16_t                  : 16;   // 8-9
} bm1373_asic_result_cmd_t;

typedef struct __attribute__((__packed__))
{
    uint16_t preamble;                // 0-1
    union {
        bm1373_asic_result_job_t job; // 2-9
        bm1373_asic_result_cmd_t cmd; // 2-9
    };
    uint8_t crc             : 5;      // 10:0-5
    uint8_t                 : 2;      // 10:6-7
    uint8_t is_job_response : 1;      // 10:8
} bm1373_asic_result_t;

static const char * TAG = "bm1373";

static task_result result;

// Fixed address interval: BM1373 uses 16 regardless of chip count
static const int address_interval = 16;

static void _send_BM1373(uint8_t header, const uint8_t * data, uint8_t data_len, bool debug)
{
    packet_type_t packet_type = (header & TYPE_JOB) ? JOB_PACKET : CMD_PACKET;
    const uint8_t total_length = (packet_type == JOB_PACKET) ? (data_len + 6) : (data_len + 5);

    uint8_t buf[total_length];

    buf[0] = 0x55;
    buf[1] = 0xAA;

    buf[2] = header;

    buf[3] = (packet_type == JOB_PACKET) ? (data_len + 4) : (data_len + 3);

    memcpy(buf + 4, data, data_len);

    if (packet_type == JOB_PACKET) {
        uint16_t crc16_total = crc16_false(buf + 2, data_len + 2);
        buf[4 + data_len] = (crc16_total >> 8) & 0xFF;
        buf[5 + data_len] = crc16_total & 0xFF;
    } else {
        buf[4 + data_len] = crc5(buf + 2, data_len + 2);
    }

    if (SERIAL_send(buf, total_length, debug) == 0) {
        ESP_LOGE(TAG, "Failed to send data to BM1373");
    }
}

static void _send_chain_inactive(void)
{
    unsigned char read_address[] = {0x00, 0x00};
    _send_BM1373((TYPE_CMD | GROUP_ALL | CMD_INACTIVE), read_address, 2, BM1373_SERIALTX_DEBUG);
}

static void _set_chip_address(uint8_t chipAddr)
{
    unsigned char read_address[] = {chipAddr, 0x00};
    _send_BM1373((TYPE_CMD | GROUP_SINGLE | CMD_SETADDRESS), read_address, 2, BM1373_SERIALTX_DEBUG);
}

void BM1373_set_version_mask(uint32_t version_mask)
{
    int versions_to_roll = version_mask >> 13;
    uint8_t version_byte0 = (versions_to_roll >> 8);
    uint8_t version_byte1 = (versions_to_roll & 0xFF);
    uint8_t version_cmd[] = {0x00, 0xA4, 0x90, 0x00, version_byte0, version_byte1};
    _send_BM1373(TYPE_CMD | GROUP_ALL | CMD_WRITE, version_cmd, 6, BM1373_SERIALTX_DEBUG);
}

void BM1373_set_hash_counting_number(uint32_t hcn)
{
    uint8_t set_10_hash_counting[6] = {0x00, 0x10, 0x00, 0x00, 0x00, 0x00};
    set_10_hash_counting[2] = (hcn >> 24) & 0xFF;
    set_10_hash_counting[3] = (hcn >> 16) & 0xFF;
    set_10_hash_counting[4] = (hcn >> 8) & 0xFF;
    set_10_hash_counting[5] = hcn & 0xFF;
    _send_BM1373((TYPE_CMD | GROUP_ALL | CMD_WRITE), set_10_hash_counting, 6, BM1373_SERIALTX_DEBUG);
}

void BM1373_set_nonce_space(double nonce_percent, float frequency, uint16_t asic_count, uint16_t cores)
{
    int cores_up = _next_power_of_two(cores);
    int asic_count_up = _next_power_of_two(asic_count);

    float hcn_space = (float)NONCE_SPACE / cores_up / asic_count_up;
    double hcn_max = hcn_space * (double)FREQ_MULT / frequency * 0.5f;
    // BM1373 has a HW errata of 134 per clock cycle; use 2x to avoid duplicates
    int hcn_error = 2 * 134;
    double hcn_frac = nonce_percent * (hcn_max - hcn_error);
    uint32_t hcn_register_value = (uint32_t)hcn_frac;

    BM1373_set_hash_counting_number(hcn_register_value);
}

float BM1373_send_hash_frequency(float target_freq)
{
    uint8_t fb_divider, refdiv, postdiv1, postdiv2;
    float frequency;

    pll_get_parameters(target_freq, 160, 239, &fb_divider, &refdiv, &postdiv1, &postdiv2, &frequency);

    uint8_t vdo_scale = (fb_divider * FREQ_MULT / refdiv >= 2400) ? 0x50 : 0x40;
    uint8_t postdiv = (((postdiv1 - 1) & 0xf) << 4) | ((postdiv2 - 1) & 0xf);
    uint8_t freqbuf[6] = {0x00, 0x08, vdo_scale, fb_divider, refdiv, postdiv};

    _send_BM1373(TYPE_CMD | GROUP_ALL | CMD_WRITE, freqbuf, 6, BM1373_SERIALTX_DEBUG);

    ESP_LOGI(TAG, "Setting Frequency to %g MHz (%g)", target_freq, frequency);

    return frequency;
}

uint8_t BM1373_init(void * pvParameters)
{
    GlobalState * GLOBAL_STATE = (GlobalState *)pvParameters;

    uint16_t asic_count = GLOBAL_STATE->DEVICE_CONFIG.family.asic_count;

    // Set version rolling mask without enable bit (0x80) before chip enumeration
    for (int i = 0; i < 3; i++) {
        _send_BM1373((TYPE_CMD | GROUP_ALL | CMD_WRITE), (uint8_t[]){0x00, 0xA4, 0x80, 0x00, 0xFF, 0xFF}, 6, BM1373_SERIALTX_DEBUG);
    }

    // Read chip ID register on all chips
    _send_BM1373((TYPE_CMD | GROUP_ALL | CMD_READ), (uint8_t[]){0x00, BM_CHIP_ID}, 2, BM1373_SERIALTX_DEBUG);

    int chip_counter = count_asic_chips(asic_count, BM1373_CHIP_ID, BM1373_CHIP_ID_RESPONSE_LENGTH);

    if (chip_counter == 0) {
        return 0;
    }

    // Version mask without enable (one more time after count)
    _send_BM1373((TYPE_CMD | GROUP_ALL | CMD_WRITE), (uint8_t[]){0x00, 0xA4, 0x80, 0x00, 0xFF, 0xFF}, 6, BM1373_SERIALTX_DEBUG);

    // Reg_A8
    _send_BM1373((TYPE_CMD | GROUP_ALL | CMD_WRITE), (uint8_t[]){0x00, 0xA8, 0x00, 0x07, 0x00, 0x00}, 6, BM1373_SERIALTX_DEBUG);

    // Misc Control (0xFF differs from BM1370's 0xF0)
    _send_BM1373((TYPE_CMD | GROUP_ALL | CMD_WRITE), (uint8_t[]){0x00, 0x18, 0xFF, 0x00, 0xC1, 0x00}, 6, BM1373_SERIALTX_DEBUG);

    // Chain inactive
    _send_chain_inactive();

    // Assign chip addresses (fixed interval of 16)
    for (uint8_t i = 0; i < chip_counter; i++) {
        _set_chip_address(i * address_interval);
    }

    // Core Register Control
    _send_BM1373((TYPE_CMD | GROUP_ALL | CMD_WRITE), (uint8_t[]){0x00, 0x3C, 0x80, 0x00, 0x80, 0x0C}, 6, BM1373_SERIALTX_DEBUG);

    uint16_t difficulty = GLOBAL_STATE->DEVICE_CONFIG.family.asic.difficulty;

    // Set difficulty mask
    uint8_t difficulty_mask[6];
    get_difficulty_mask(difficulty, difficulty_mask);
    _send_BM1373((TYPE_CMD | GROUP_ALL | CMD_WRITE), difficulty_mask, 6, BM1373_SERIALTX_DEBUG);

    // Set the IO Driver Strength
    _send_BM1373((TYPE_CMD | GROUP_ALL | CMD_WRITE), (uint8_t[]){0x00, 0x58, 0x00, 0x01, 0x11, 0x11}, 6, BM1373_SERIALTX_DEBUG);

    // PLL3 Parameter
    _send_BM1373((TYPE_CMD | GROUP_ALL | CMD_WRITE), (uint8_t[]){0x00, 0x68, 0x5A, 0xA5, 0x5A, 0xA5}, 6, BM1373_SERIALTX_DEBUG);

    for (uint8_t i = 0; i < chip_counter; i++) {
        uint8_t addr = i * address_interval;
        // Reg_A8
        unsigned char set_a8_register[6] = {addr, 0xA8, 0x00, 0x07, 0x01, 0xF0};
        _send_BM1373((TYPE_CMD | GROUP_SINGLE | CMD_WRITE), set_a8_register, 6, BM1373_SERIALTX_DEBUG);
        // Misc Control (0xFF)
        unsigned char set_18_register[6] = {addr, 0x18, 0xFF, 0x00, 0xC1, 0x00};
        _send_BM1373((TYPE_CMD | GROUP_SINGLE | CMD_WRITE), set_18_register, 6, BM1373_SERIALTX_DEBUG);
        // Core Register Control
        unsigned char set_3c_register_first[6] = {addr, 0x3C, 0x80, 0x00, 0x80, 0x0C};
        _send_BM1373((TYPE_CMD | GROUP_SINGLE | CMD_WRITE), set_3c_register_first, 6, BM1373_SERIALTX_DEBUG);
        // Core Register Control
        unsigned char set_3c_register_second[6] = {addr, 0x3C, 0x80, 0x00, 0x82, 0xAA};
        _send_BM1373((TYPE_CMD | GROUP_SINGLE | CMD_WRITE), set_3c_register_second, 6, BM1373_SERIALTX_DEBUG);
    }

    // Analog Mux Control
    _send_BM1373((TYPE_CMD | GROUP_ALL | CMD_WRITE), (uint8_t[]){0x00, 0x54, 0x00, 0x00, 0x00, 0x02}, 6, BM1373_SERIALTX_DEBUG);

    // Ramp up the hash frequency
    do_frequency_transition(GLOBAL_STATE, BM1373_send_hash_frequency);

    float frequency = GLOBAL_STATE->POWER_MANAGEMENT_MODULE.frequency_value;
    int cores = GLOBAL_STATE->DEVICE_CONFIG.family.asic.core_count;

    BM1373_set_nonce_space(1.0, frequency, asic_count, cores);

    // Enable version rolling now that the chain is fully configured
    _send_BM1373((TYPE_CMD | GROUP_ALL | CMD_WRITE), (uint8_t[]){0x00, 0xA4, 0x90, 0x00, 0xFF, 0xFF}, 6, BM1373_SERIALTX_DEBUG);

    return chip_counter;
}

int BM1373_set_default_baud(void)
{
    // default divider of 26 (11010) for 115,749
    unsigned char baudrate[] = {0x00, MISC_CONTROL, 0x00, 0x00, 0b01111010, 0b00110001};
    _send_BM1373((TYPE_CMD | GROUP_ALL | CMD_WRITE), baudrate, 6, BM1373_SERIALTX_DEBUG);
    return 115749;
}

int BM1373_set_max_baud(void)
{
    ESP_LOGI(TAG, "Setting max baud of 1000000 ");

    unsigned char fast_uart[] = {0x00, FAST_UART_CONFIGURATION, 0x11, 0x30, 0x02, 0x00};
    _send_BM1373((TYPE_CMD | GROUP_ALL | CMD_WRITE), fast_uart, 6, BM1373_SERIALTX_DEBUG);
    return 1000000;
}

static uint8_t id = 0;

void BM1373_send_work(void * pvParameters, bm_job * next_bm_job)
{
    GlobalState * GLOBAL_STATE = (GlobalState *) pvParameters;

    BM1373_job job;
    id = (id + 24) % 128;
    job.job_id = id;
    job.num_midstates = 0x01;
    memcpy(&job.starting_nonce, &next_bm_job->starting_nonce, 4);
    memcpy(&job.nbits, &next_bm_job->target, 4);
    memcpy(&job.ntime, &next_bm_job->ntime, 4);
    memcpy(job.merkle_root, next_bm_job->merkle_root, 32);
    memcpy(job.prev_block_hash, next_bm_job->prev_block_hash, 32);
    memcpy(&job.version, &next_bm_job->version, 4);

    if (GLOBAL_STATE->ASIC_TASK_MODULE.active_jobs[job.job_id] != NULL) {
        free_bm_job(GLOBAL_STATE->ASIC_TASK_MODULE.active_jobs[job.job_id]);
    }

    GLOBAL_STATE->ASIC_TASK_MODULE.active_jobs[job.job_id] = next_bm_job;

    pthread_mutex_lock(&GLOBAL_STATE->valid_jobs_lock);
    GLOBAL_STATE->valid_jobs[job.job_id] = 1;
    pthread_mutex_unlock(&GLOBAL_STATE->valid_jobs_lock);

    #if BM1373_DEBUG_JOBS
    ESP_LOGI(TAG, "Send Job: %02X", job.job_id);
    #endif

    _send_BM1373((TYPE_JOB | GROUP_SINGLE | CMD_WRITE), (uint8_t *)&job, sizeof(BM1373_job), BM1373_DEBUG_WORK);
}

task_result * BM1373_process_work(void * pvParameters)
{
    bm1373_asic_result_t asic_result = {0};

    memset(&result, 0, sizeof(task_result));

    if (receive_work((uint8_t *)&asic_result, sizeof(asic_result), &result.timestamp_us) == ESP_FAIL) {
        return NULL;
    }

    if (!asic_result.is_job_response) {
        result.register_type = REGISTER_MAP[asic_result.cmd.register_address];
        if (result.register_type == REGISTER_INVALID) {
            ESP_LOGW(TAG, "Unknown register read: %02x", asic_result.cmd.register_address);
            return NULL;
        }
        result.asic_nr = asic_result.cmd.asic_address / address_interval;
        result.value = ntohl(asic_result.cmd.value);

        return &result;
    }

    uint8_t job_id = (asic_result.job.id & 0xf0) >> 1;
    uint32_t nonce_h = ntohl(asic_result.job.nonce);
    // BM1373 encodes ASIC index in bits 24-25 of the byte-swapped nonce
    uint8_t asic_nr = (uint8_t)((nonce_h >> 24) & 0x03);
    uint8_t core_id = (uint8_t)((nonce_h >> 26) & 0x7f); // 7 bits for up to 128 cores
    uint8_t small_core_id = asic_result.job.id & 0x0f;
    uint32_t version_bits = (ntohs(asic_result.job.version) << 13);

    GlobalState * GLOBAL_STATE = (GlobalState *) pvParameters;

    if (GLOBAL_STATE->valid_jobs[job_id] == 0) {
        ESP_LOGW(TAG, "Invalid job nonce found, 0x%02X", job_id);
        return NULL;
    }

    uint32_t rolled_version = GLOBAL_STATE->ASIC_TASK_MODULE.active_jobs[job_id]->version | version_bits;

    result.job_id = job_id;
    result.nonce = asic_result.job.nonce;
    result.rolled_version = rolled_version;
    result.asic_nr = asic_nr;
    result.core_id = core_id;
    result.small_core_id = small_core_id;

    return &result;
}

void BM1373_read_registers(void)
{
    int size = sizeof(REGISTER_MAP) / sizeof(REGISTER_MAP[0]);
    for (int reg = 0; reg < size; reg++) {
        if (REGISTER_MAP[reg] != REGISTER_INVALID) {
            _send_BM1373((TYPE_CMD | GROUP_ALL | CMD_READ), (uint8_t[]){0x00, reg}, 2, BM1373_SERIALTX_DEBUG);
            vTaskDelay(1 / portTICK_PERIOD_MS);
        }
    }
}
