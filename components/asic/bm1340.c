...
#define BM1340_CHIP_ID 0x1340
#define BM1340_CHIP_ID_RESPONSE_LENGTH 11
...
typedef struct __attribute__((__packed__))
{
    ...
    uint16_t : 16;
} bm1340_asic_result_cmd_t;

typedef struct __attribute__((__packed__))
{
    uint16_t preamble;
    union {
        bm1340_asic_result_job_t job;
        bm1340_asic_result_cmd_t cmd;
    };
    uint8_t crc             : 5;
    uint8_t                 : 2;
    uint8_t is_job_response : 1;
} bm1340_asic_result_t;

static const char * TAG = "bm1340";
...
/// @param data
/// @param len
static void _send_BM1340(uint8_t header, uint8_t * data, uint8_t data_len, bool debug)
{
    ...
    if (SERIAL_send(buf, total_length, debug) == 0) {
        ESP_LOGE(TAG, "Failed to send data to BM1340");
    }
...
static void _send_simple(uint8_t * data, uint8_t total_length)
{
    ...
    SERIAL_send(buf, total_length, BM1340_SERIALTX_DEBUG);
...
static void _send_chain_inactive(void)
{
    ...
    // send serial data
    _send_BM1340((TYPE_CMD | GROUP_ALL | CMD_INACTIVE), read_address, 2, BM1340_SERIALTX_DEBUG);
}
...
static void _set_chip_address(uint8_t chipAddr)
{
    ...
    // send serial data
    _send_BM1340((TYPE_CMD | GROUP_SINGLE | CMD_SETADDRESS), read_address, 2, BM1340_SERIALTX_DEBUG);
}
...
void BM1340_set_nonce_space(double nonce_percent, float frequency, uint16_t asic_count, uint16_t cores)
{
    ...
    BM1340_set_hash_counting_number(hcn_register_value);
}

float BM1340_send_hash_frequency(float target_freq) 
{
    ...
    _send_BM1340(TYPE_CMD | GROUP_ALL | CMD_WRITE, freqbuf, 6, BM1340_SERIALTX_DEBUG);
...
uint8_t BM1340_init(void * pvParameters)
{
    ...
    int chip_counter = count_asic_chips(asic_count, BM1340_CHIP_ID, BM1340_CHIP_ID_RESPONSE_LENGTH);
    ...
    _send_BM1340((TYPE_CMD | GROUP_ALL | CMD_WRITE), (uint8_t[]){0x00, 0xA8, 0x00, 0x07, 0x00, 0x00}, 6, BM1340_SERIALTX_DEBUG);

    //Misc Control
    //TX: 55 AA 51 09 [00 18 F0 00 C1 00] 04 //command all chips, write chip address 00, register 18, data F0 00 C1 00 - Misc Control
    _send_BM1340((TYPE_CMD | GROUP_ALL | CMD_WRITE), (uint8_t[]){0x00, 0x18, 0xFF, 0x00, 0xC1, 0x00}, 6, BM1340_SERIALTX_DEBUG); //from S21Pro dump
    //_send_BM1340((TYPE_CMD | GROUP_ALL | CMD_WRITE), (uint8_t[]){0x00, 0x18, 0xFF, 0x0F, 0xC1, 0x00}, 6, BM1340_SERIALTX_DEBUG); //from S21 dump
    ...
    _send_BM1340((TYPE_CMD | GROUP_ALL | CMD_WRITE), (uint8_t[]){0x00, 0x3C, 0x80, 0x00, 0x80, 0x0C}, 6, BM1340_SERIALTX_DEBUG); //from S21Pro dump
    //_send_BM1340((TYPE_CMD | GROUP_ALL | CMD_WRITE), (uint8_t[]){0x00, 0x3C, 0x80, 0x00, 0x80, 0x18}, 6, BM1340_SERIALTX_DEBUG); //from S21 dump

    //set difficulty mask
    uint16_t difficulty = GLOBAL_STATE->DEVICE_CONFIG.family.asic.difficulty;
    uint8_t difficulty_mask[6];
    get_difficulty_mask(difficulty, difficulty_mask);
    _send_BM1340((TYPE_CMD | GROUP_ALL | CMD_WRITE), difficulty_mask, 6, BM1340_SERIALTX_DEBUG);    
...
//     unsigned char read_address[2] = {0x00, 0x00};
//     // send serial data
//     _send_BM1340((TYPE_CMD | GROUP_ALL | CMD_READ), read_address, 2, BM1340_SERIALTX_DEBUG);
// }
...
int BM1340_set_default_baud(void)
{
    ...
    _send_BM1340((TYPE_CMD | GROUP_ALL | CMD_WRITE), baudrate, 6, BM1340_SERIALTX_DEBUG);
    return 115749;
}
...
void BM1340_send_work(void * pvParameters, bm_job * next_bm_job)
{
    GlobalState * GLOBAL_STATE = (GlobalState *) pvParameters;
    ...
    #if BM1340_DEBUG_JOBS
    ESP_LOGI(TAG, "Send Job: %02X", job.job_id);
    #endif

    _send_BM1340((TYPE_JOB | GROUP_SINGLE | CMD_WRITE), (uint8_t *)&job, sizeof(BM1340_job), BM1340_DEBUG_WORK);
}

task_result * BM1340_process_work(void * pvParameters)
{
    bm1340_asic_result_t asic_result = {0};
...
void BM1340_read_registers(void)
{
    int size = sizeof(REGISTER_MAP) / sizeof(REGISTER_MAP[0]);
    for (int reg = 0; reg < size; reg++) {
        if (REGISTER_MAP[reg] != REGISTER_INVALID) {
            _send_BM1340((TYPE_CMD | GROUP_ALL | CMD_READ), (uint8_t[]){0x00, reg}, 2, BM1340_SERIALTX_DEBUG);
