...
void BM1340_send_hash_frequency(float target_freq) 
{
...
uint8_t BM1340_init(float frequency, uint16_t asic_count, uint16_t difficulty)
{
    ...
    _send_BM1340((TYPE_CMD | GROUP_ALL | CMD_WRITE), (uint8_t[]){0x00, 0x18, 0xF0, 0x00, 0xC1, 0x00}, 6, BM1340_SERIALTX_DEBUG); //from S21Pro dump
...
task_result * BM1340_process_work(void * pvParameters)
{
    ...
    uint8_t core_id = (uint8_t)((ntohl(asic_result.nonce) >> 25) & 0x7f); // BM1340 has 80 cores, so it should be coded on 7 bits
    uint8_t small_core_id = asic_result.job_id & 0x0f; // BM1340 has 16 small cores, so it should be coded on 4 bits
