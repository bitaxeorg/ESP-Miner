...
void STRATUM_V1_parse(StratumApiV1Message * message, const char * stratum_json)
{
    ...
    if (message->method == MINING_NOTIFY) {
        ...
        message->should_abandon_work = params_count > 8 ? cJSON_IsTrue(cJSON_GetArrayItem(params, params_count - 1)) : false;
...
int STRATUM_V1_subscribe(int socket, int send_uid, const char * model)
{
    ...
    snprintf(subscribe_msg, sizeof(subscribe_msg), "{\"id\": %d, \"method\": \"mining.subscribe\", \"params\": [\"bitaxe/%s/%s\"]}\n", send_uid, model, version);
...
int STRATUM_V1_extranonce_subscribe(int socket, int send_uid)
{
    ...
    snprintf(extranonce_msg, sizeof(extranonce_msg), "{\"id\": %d, \"method\": \"mining.extranonce.subscribe\", \"params\": []}\n", send_uid);
                            ...
                            const uint32_t nonce, const uint32_t version_bits)
{
    ...
    int ret = write(socket, submit_msg, strlen(submit_msg));
