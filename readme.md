...
```
bitaxetool --config ./configs/config-401.cvs
```
...
```
bitaxetool --config ./configs/config-401.cvs --firmware ./esp-miner-factory-401-v2.4.2.bin
```
...
```
bitaxetool --config ./configs/config-xxx.cvs --firmware ./esp-miner-merged.bin
```
...
A custom board version is also possible with `configs/config-custom.cvs`. A custom board needs to be based on an existing `devicemodel` and `asicmodel`.
