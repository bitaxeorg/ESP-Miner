...
def _process_device(ip: str, build_dir: pathlib.Path, password: str | None = None) -> bool:
    """Upload web UI then firmware to *ip*. Return True if both succeed."""
    if not _verify_auth(ip, password):
        return False
    www_ok = _upload_binary(ip, build_dir / _WWW_BIN, _ENDPOINT_WWW, password)
    if not www_ok:
        return False
...
def main(argv: List[str] | None = None) -> None:
    ...
    for ip in ips:
        ...
        success = _process_device(ip, build_dir, password)
