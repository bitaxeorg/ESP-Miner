...
static esp_err_t PATCH_update_settings(httpd_req_t * req)
{
    ...
    if (check_json_type((cJSON_Number | cJSON_True | cJSON_False), item = cJSON_GetObjectItem(root, "flipscreen"))) {
    ...
    if (check_json_type(cJSON_Number, item = cJSON_GetObjectItem(root, "statsLimit"))) {
        nvs_config_set_u16(NVS_CONFIG_STATISTICS_LIMIT, item->valueint);
    }
    if (check_json_type(cJSON_Number, item = cJSON_GetObjectItem(root, "statsDuration"))) {
