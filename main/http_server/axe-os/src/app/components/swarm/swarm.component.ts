...
export class SwarmComponent implements OnInit, OnDestroy {
  ...
  public add() {
    ...
    forkJoin({
      ...
      // Check if the device is already in swarm via hostname or IP
      if (this.swarm.some(item => item.address === info['hostname'] || item.address === info['ip'])) {
        this.toastr.warning('This device is already in the swarm.');
        return;
      }
  ...
  public postAction(device: any, action: string) {
    this.httpClient.post(`http://${device.address}/api/system/${action}`, {}, { responseType: 'text' }).pipe(
      timeout(800),
      catchError(error => {
  ...
  public refreshList(fetchAsic: boolean = true) {
    ...
    this.refreshIntervalTime = this.refreshTimeSet;
    const addresses = this.swarm.filter(Boolean).map(axeOs => axeOs.address);
