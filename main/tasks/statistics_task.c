...
void createStatisticsBuffer()
{
    if (NULL == statisticsBuffer) {
        ...
        if (NULL == statisticsBuffer) {
            statisticsBuffer = (StatisticsDataPtr)heap_caps_malloc(sizeof(struct StatisticsData) * maxDataCount, MALLOC_CAP_DEFAULT | MALLOC_CAP_SPIRAM);
            if (NULL == statisticsBuffer) {
...
bool getStatisticData(uint16_t index, StatisticsDataPtr dataOut)
{
    ...
    if ((NULL != statisticsBuffer) && (statisticsDataSize > index)) {
